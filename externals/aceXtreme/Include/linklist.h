 ...
 *
 *
 *  linklist.h (Generic Double Link List header)
 *
...
DLISTNODE* _DECL _llCreateNode( S16BIT nType,
                                S16BIT nID);

/* _llDeleteNode -> free's a node */
void _DECL _llDeleteNode(DLISTNODE** ppNode);

/* _llDeleteAsyncNode -> free's a node */
void _DECL _llDeleteAsyncNode(ALISTNODE** ppNode);

/* _llGetNextNode -> gets the next node in the list */
DLISTNODE* _DECL _llGetNextNode(DLISTNODE* pNode);

/* _llGetPrevNode -> gets the previous node in the list */
DLISTNODE* _DECL _llGetPrevNode(DLISTNODE* pNode);

/* _llInsertNodeAt -> inserts a node after a given node */
S16BIT _DECL _llInsertNodeAt(   DLISTNODE** ppHead,
                                DLISTNODE* pPrevNode,
                                DLISTNODE* pNode);

/* _llInsertNodeSorted -> inserts node into a sorted list based on ID */
S16BIT _DECL _llInsertNodeSorted(   DLISTNODE** ppHead,
                                    DLISTNODE* pNode);

/* _llRemoveAllNodes -> removes all nodes from a list */
S16BIT _DECL _llRemoveAllNodes(DLISTNODE** ppHead);

/* _llRemoveDeleteNodeFromID -> removes and deletes from list given ID */
S16BIT _DECL _llRemoveDeleteNodeFromID( DLISTNODE** ppHead,
                                        S16BIT nID,
                                        S16BIT nType);

/* _llRemoveNode -> removes a node from a list (no delete) */
S16BIT _DECL _llRemoveNode( DLISTNODE** ppHead,
                            DLISTNODE* pNode);

/* _llSearchForNode -> attempts to find a node in a list given ID,Type */
S16BIT _DECL _llSearchForNode(  DLISTNODE* pHead,
                                S16BIT nID,
                                S16BIT nType,
                                DLISTNODE** ppSearch,
                                U16BIT bSortedList);
...
ALISTNODE* _DECL _llCreateAsyncNode(S16BIT nID);

/* _llInsertAsyncNode -> inserts node to end of sorted list */
S16BIT _DECL _llInsertAsyncNode(ALISTNODE** ppHead,
                   ALISTNODE* pNode);

/* _llSearchForAysncNode -> search for node in list */
S16BIT _DECL _llSearchForAsyncNode( ALISTNODE* pHead,
                                    S16BIT nMsgID,
                                    ALISTNODE** ppSearch);

/* _llSearchForAsyncNodebyOp -> search for node in list */
S16BIT _DECL _llSearchForAsyncNodebyOp(ALISTNODE* pHead,
                                      S16BIT nOpID,
                                      ALISTNODE** ppSearch);

/* _llRemoveAsyncNodeFromID -> removes and deletes from list given ID */
S16BIT _DECL _llRemoveAsyncNodeFromID(  ALISTNODE** ppHead,
                                        S16BIT nID);

/* _llRemoveAllAsyncNodes -> removes all async nodes from a list */
S16BIT _DECL _llRemoveAllAsyncNodes(ALISTNODE** ppHead);

/* _llCreateFrameNode -> malloc's a new frame node */
FLISTNODE* _DECL _llCreateFrameNode(S16BIT nID);

/* _llInsertFrameNode -> inserts node to end list */
S16BIT _DECL _llInsertFrameNode(FLISTNODE** ppHead,
                   FLISTNODE* pNode);

/* _llSearchForFrameNode -> search for node in list */
S16BIT _DECL _llSearchForFrameNode( FLISTNODE* pHead,
                                    S16BIT nMsgID,
                                    FLISTNODE** ppSearch);

/* _llRemoveAllFrameNodes -> removes all async nodes from a list */
S16BIT _DECL _llRemoveAllFrameNodes(FLISTNODE** ppHead);
