...
* Module Description:
*
* The purpose of this module is to define memory manager functions and definitions.
*
*****************************************************************************/
...
#define ADDR_UNITS_DATA(CARD_TYPE,DATA_WORDS) ((U16BIT)(CARD_TYPE & ACE_FLEX_DEVICE ? ((DATA_WORDS / 2)+(DATA_WORDS % 2)) : DATA_WORDS))

/* Memory Word To Byte Calculation Macro - Memory */
#define ADDR_UNITS_TO_BYTES(CARD_TYPE,ADDR_UNITS) (CARD_TYPE & ACE_FLEX_DEVICE ? (ADDR_UNITS * sizeof(U32BIT)) : (ADDR_UNITS * sizeof(U16BIT)))
...
/* MultiRT specific: each RT has Control Block in low mem
                     and Data/CMD block in high mem areas due to 16bit
                     and 19bit registers   */
...
S16BIT _DECL _mmAllocMemBlock(  MEMBLK_LIST *pMemBlkList,
                                S16BIT nID,
                                S16BIT nType,
                                S16BIT nMemFlag,
                                U32BIT dwSize,
...
S16BIT _DECL _mmFindMemBlock(   MEMBLK_LIST *pMemBlkList,
                                S16BIT nID,
                                S16BIT nType,
                                DLISTNODE** ppSearch);

/* _mmFreeMemBlocksList -> frees up all memory blocks in memory manager */
S16BIT _DECL _mmFreeMemBlockList(MEMBLK_LIST *pMemBlkList);

/* _mmInitMemBlockList -> initializes the libraries memory manager */
S16BIT _DECL _mmInitMemBlockList(MEMBLK_LIST *pMemBlkList);

/* _mmReleaseMemBlock -> frees up a allocated (used) memory block */
S16BIT _DECL _mmReleaseMemBlock(MEMBLK_LIST *pMemBlkList,
                                S16BIT nID,
                                S16BIT nType/*,   **POSSIBLE WAY TO ADDRESS OPCODE CASE CDF ****
                                DLISTNODE *pOpCodes*/);

/* _mmResetFreeMemBlockPtr -> sets the internal free mem block ptr to
 *      first free memory block in list.
 */
S16BIT _DECL _mmResetFreeMemBlockPtr(MEMBLK_LIST *pMemBlkList);

/* _mmReleaseModMemBlocks -> removes module-specific memory blocks */
S16BIT _DECL _mmReleaseModMemBlocks(MEMBLK_LIST *pMemBlkList, U8BIT u8Mod);
